<script>
    ...
    // 素材の読み込み
    k.loadSprite("floor", "src/images/floor.png");
    k.loadSprite("wall", "src/images/wall.png");
    k.loadSprite("player", "src/images/player.png");
    k.loadSprite("monster", "src/images/slime.png");
    k.loadSprite("stairs", "src/images/stairs.png");
    k.loadSprite("potion", "src/images/potion.png");
    k.loadSprite("scroll", "src/images/scroll.png");

    k.loadSound("hit", "src/sounds/hit.mp3");
    k.loadSound("slash", "src/sounds/slash.mp3");
    ...
    k.scene("main", () => {
        ...
        const moveEnemies = () => {
            ...
            monsters.forEach(m => {
                ...
                if (nextX === px && nextY === py) {
                    addLog("モンスターの攻撃！");
                    k.play("hit", { volume: 0.1 });
                    player.hp -= floorLevel;
                    addLog(`プレイヤーに ${floorLevel} のダメージ！`);
        ...
        function endTurn() {
            ...
            if (player.hp <= 0) {
                ...
                /** ここでリスタートなどの処理を入れる */
                inventory = []; // 持ち物をリセット
                k.go("gameover", floorLevel); // 現在の階層をスコアとして渡す
            }
    ...
    k.scene("gameover", (score) => {
        ...
        k.add([
            k.text(`到達階層: 地下 ${score} 階`, { size: 24 }),
            k.pos(k.width() / 2, k.height() / 2 + 60),
            k.anchor("center"),
        ]);

        k.add([
            k.text("Press Space to Retry", { size: 18 }),
